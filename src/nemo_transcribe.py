...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--runtime",
        required=True,
        choices=["parakeet_nemo", "canary_nemo"],
    )
...
def main() -> int:
    ...
    print(f"[nemo-local] runtime={args.runtime} model={args.model_id} device={device}", file=sys.stderr)

    try:
        if args.runtime == "parakeet_nemo":
        ...
        print(f"[nemo-local] transcription failed: {exc}", file=sys.stderr)
        return 1
